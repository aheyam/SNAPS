...
if "generic_preds" in args.test or "all" in args.test:
    ...
    if args.assign:
        ...
        for i in id_all:
            print(testset_df.loc[i, "out_name"])
            cmd = make_cmd(i, out_dir, "test/config_generic_preds.yaml", ["--strip_plot"])
            run(cmd)
    ...
    if args.analyse:
        ...
        assigns_generic_preds = collect_assignment_results(path/"output"/out_dir, testset_df, ID_list=id_all)
